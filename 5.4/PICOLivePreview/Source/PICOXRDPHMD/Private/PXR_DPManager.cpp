	...
	case EGraphicQuality::EPIC:
		{
			...
			streamingConfig.view_width = 2160;
			streamingConfig.view_height = 2160;
		}
	...
	case EGraphicQuality::Medium:
		{
			...
			streamingConfig.view_width = 1440;
			streamingConfig.view_height = 1440;
		}
		break;
		// 低质量(50%)
	case EGraphicQuality::Low:
		{
			...
			streamingConfig.view_width = 1200;
			streamingConfig.view_height = 1200;
		}
		break;
	default:
		;
...
bool FPICOXRDPManager::CreateSharedTexture2D()
{
	...
	sharedTexture2Ddesc.MiscFlags = D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX;
	
	//uint32_t m_color = ColorRGBA(255, 108, 108, 255);
	uint32_t m_color = ColorRGBA(255, 255, 255, 255);
	...
	case EGraphicQuality::EPIC:
		{
			//IdealRenderTargetSize = FIntPoint(4230, 2160);
			sharedTexture2Ddesc.Height = 2160; //m_lastSize.Height;
			sharedTexture2Ddesc.Width = 2160; // m_lastSize.Width;
			textureMap.assign(2160 * 2160, m_color);
			uint32_t* pData = textureMap.data();
			sd.pSysMem = pData;
			sd.SysMemPitch = 2160 * sizeof(uint32_t);
			sd.SysMemSlicePitch = 2160 * 2160 * sizeof(uint32_t);
		}
	case EGraphicQuality::High:
		{
			//IdealRenderTargetSize = FIntPoint(3840, 1920);
			...
			textureMap.assign(1920 * 1920, m_color);
			uint32_t* pData = textureMap.data();
			sd.pSysMem = pData;
	...
	case EGraphicQuality::Medium:
		{
			//IdealRenderTargetSize = FIntPoint(2880, 1440);
			sharedTexture2Ddesc.Height = 1440; //m_lastSize.Height;
			sharedTexture2Ddesc.Width = 1440; // m_lastSize.Width;
			textureMap.assign(1440 * 1400, m_color);
			uint32_t* pData = textureMap.data();
			sd.pSysMem = pData;
			sd.SysMemPitch = 1440 * sizeof(uint32_t);
			sd.SysMemSlicePitch = 1440 * 1440 * sizeof(uint32_t);
		}
		break;
		// 低质量(50%)
	case EGraphicQuality::Low:
		{
			// 这个地方写错了，虽然是拷贝但是，是平铺的。所以不对
			//IdealRenderTargetSize = FIntPoint(2400, 1200);
			sharedTexture2Ddesc.Height = 1200; //m_lastSize.Height;
			sharedTexture2Ddesc.Width = 1200; // m_lastSize.Width;
			textureMap.assign(1200 * 1200, m_color);
			uint32_t* pData = textureMap.data();
			sd.pSysMem = pData;
			sd.SysMemPitch = 1200 * sizeof(uint32_t);
			sd.SysMemSlicePitch = 1200 * 1200 * sizeof(uint32_t);
		}
		break;
	default:
		;
...
bool FPICOXRDPManager::CreateRHITexture(ID3D11Texture2D* OpenedSharedResource, EPixelFormat Format, FTexture2DRHIRef& OutTexture)
{
	...
	ETextureCreateFlags Flags = ETextureCreateFlags::ShaderResource|ETextureCreateFlags::RenderTargetable|ETextureCreateFlags::SRGB;
	
	OutTexture = DynamicRHI->RHICreateTexture2DFromResource(
		Format, Flags, FClearValueBinding(), OpenedSharedResource).GetReference();
...
void FPICOXRDPManager::GetHMDPositionAndRotation(FVector& OutPostion, FQuat& OutQuat)
{
	if (StreamingDataWrapperPtr)
	{
		...
		// FQuat XAxisRotate = FQuat(FVector::XAxisVector, FMath::DegreesToRadians(-90.0f));
		// OutQuat = XAxisRotate * OutQuat;
	}
	else
	{
...
void FPICOXRDPManager::GetControllerPositionAndRotation(EControllerHand DeviceHand, float WorldScale, FVector& OutPostion, FRotator& OutQuat)
{
	if (StreamingDataWrapperPtr)
	{
		...
		// FQuat ControllerQuat = FQuat(ControllerRotation.z(), -ControllerRotation.x(), -ControllerRotation.y(), ControllerRotation.w());
		// FQuat XAxisRotate = FQuat(FVector::YAxisVector, FMath::DegreesToRadians(-45.0f));
		// OutQuat = (XAxisRotate * ControllerQuat).Rotator();
